  ...
  <div class="select-member">
    <div v-clickoutside="closeUserSelection">
      <div class="selected-member" v-for="(obj, index) in selectedUsers" :key="index">
        <Poptip trigger="hover" width="240">
          <Avatar :icon="obj.userName ? '' : 'person'" :src="obj.headPic">{{obj.userName ? obj.userName.substr(0, 2) : ''}}</Avatar>
          <div slot="title">
            <div class="selected-member-detail g-flex">
              <!--<Avatar icon="person" :src="obj.headPic">{{}}</Avatar>-->
              <Avatar :icon="obj.userName ? '' : 'person'" :src="obj.headPic">{{obj.userName ? obj.userName.substr(0, 2) : ''}}</Avatar>
        ...
        <div class="result" v-else>
          <ul>
            <li v-for="(item, index) in result" :key="index" @click="selectUser(item)">
              <Avatar icon="person" :src="item.headPic"></Avatar>{{item.userName}}
            </li>
          </ul>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      openUserSelection () {
        let data = {
          projectId: this.$route.params.projectId || this.projectId
        }
        this.$http.post(Task.user, this.qs.stringify(data), {headers: {'Authorization': 'Bearer ' + this.$cookie.get('token')}}).then((res) => {
      ...
      selectUser (userObj) {
        if (this.multi) {
          ...
          if (this.isN1 && userObj['N+1'] instanceof Object) {
            this.selectedUsers = [userObj, userObj['N+1']]
            this.$emit('on-change', this.selectedUsers)
          } else {
