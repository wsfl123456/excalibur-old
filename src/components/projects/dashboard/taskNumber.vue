<template>
  <div class="task-execute">
    ...
    <div class="filter">
      <div>
        <RadioGroup v-model="taskNumStatus" type="button" size="large">
    ...
    <div class="chart">
      <month-chart v-if="taskNumStatus == '按月份显示'" :info='monthData'></month-chart>
...
<script>
import MonthChart from "./numMonthChart";
import DepartChart from "./numDepartChart";
...
export default {
  ...
  computed: {
    ...
    filter() {
      const { startTime, endTime, taskNumStatus } = this
      return { startTime, endTime, taskNumStatus };
    }
  },
  watch: {
    filter: {
  ...
  methods: {
    ...
    getMonthData() {
      if (this.canSubmit) {
        ...
        const api = this.taskNumStatus == '按月份显示' ? Dashborad.projectexecmonthtasknumber : Dashborad.projectexecdepartmenttasknumber
        this.$http
          .post(
            api,
          ...
          .then(res => {
            if (this.taskNumStatus == '按月份显示') this.monthData = res.data.retData.chartResult;
