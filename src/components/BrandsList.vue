          ...
          <tr @click="toggleRow(item, index)">
            <td v-for="(_item, _index) in tableTitles" :key="'td-' + index + '-' + _index">
              <template v-if="_item.key === 'workState'">
                <div class="table-cell">{{item.compType}}</div>
              </template>
              <template v-else-if="_item.key === 'industry'">
                <div class="table-cell">{{item.industryName}}</div>
              </template>
              <template v-else-if="_item.key === 'creatorId'">
                <div class="table-cell">
                  <Avatar
                    :icon="item.creatorName ? '' : 'person'"
                    :src="item.creatorHeadPic"
                  >{{item.creatorName ? item.creatorName.substr(0, 2) : ''}}</Avatar>
                </div>
              </template>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("changeBrandDetail", this.$route.query);
    this.$store.dispatch("changeBrandDetailStatus", true);
  }
};
</script>
