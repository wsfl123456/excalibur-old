<template>
  <div class="edit-out">
    ...
    <div v-for="(value, key) in formInfoData" :key="'taskFormData-' + key">
      <div>
        <i-form label-position="left" :label-width="88" class="default-form" >
          <div class="form-group">
            <template v-for="(value, key) in value">
              <div >
                <FormItem  :label="value.label" class="default-form-item" :required="value.required === 'true'">
                  ...
                  <DatePicker transfer v-else-if="value.type === 'datetime'" :time-picker-options="value.workTime ? timeOpts : null" format="yyyy-MM-dd HH:mm" :options="dateOpt" class="deadline" :type="value.type" :placeholder="value.placeholder" :value="value.value" :clearable="false" @on-change="changeDataDate($event, key)"></DatePicker>
                  <DatePicker transfer v-else-if="value.type === 'date'" :options="dateOpt" class="deadline" :type="value.type" :placeholder="value.placeholder" :value="value.value" :clearable="false" @on-change="changeDataDate($event, key)"></DatePicker>
                ...
                <Row>
                  <i-col span="12">
                    <el-select v-model="customerId" placeholder="品牌选择" @change="changeCustomer"  :clearable="false" filterable>
                      <el-option v-for="item in allCustomer" :value="String(item.id)" :label='item.customerName' :key="item.id"></el-option>
                    </el-select>
                  </i-col>
                  <i-col span="12" class="default-form-member-col" v-if="allProject">
                    <el-select v-model="projectId" placeholder="项目选择"  :clearable="false" filterable>
                      <template v-if="customerId && allProject.length > 0">
                        <el-option v-for="item in allProject" :value="String(item.id)" :label='item.name' :key="item.id"></el-option>
                      </template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail () {
        this.isLoading = true
        this.$http.post(Task.detail, this.qs.stringify({taskId: this.$route.params.id}), {headers: {'Authorization': 'Bearer ' + this.$cookie.get('token')}}).then((res) => {
          ...
          if (_data.retCode === 0) {
            ...
            this.customerId = _data.retData.data.formInfo[0].formContent.customerId
            this.projectId = _data.retData.data.formInfo[0].formContent.projectId
            this.projectConf.customerId = _data.retData.data.formInfo[0].formContent.customerId
            this.getProject()
          } else {
            this.isToast = true
        ...
        if (this.hasOwnProperty('formInfoData') && this.formInfoData) {
          for (let i = 0; i < this.formInfoData.length; i++) {
            ...
            for (let j = 0; j < item.length; j++) {
              let _item = item[j]
              if ((_item.hasOwnProperty('required') && _item.required === 'true') && (_item.value.length === 0 || !_item.value)) {
