<template>
  <div>
    <div class="g-mb-32">
      <Row>
        <i-col span="14">
          <i-input placeholder="请输入姓名、电话、邮箱按 回车 进行查询" v-model="searchValue" @on-keydown="search">
            <span slot="prepend"><Icon type="ios-search-strong" size="20"></Icon></span>
          </i-input>
        </i-col>
        <i-col span="9" offset="1">
          <Upload class="upload-btn" name="excelFile" :action="uploadApi" accept=".xlsx, .xls" :headers="{'Authorization': 'Bearer ' + $cookie.get('token')}" :show-upload-list="false" :on-success="uploadSuccess">
            <Button type="primary" >批量导入</Button>
          </Upload>
          <Button type="primary" @click.native="exportBasic">批量导出</Button>
    ...
    <div class="result-list">
      ...
      <table class="main-list common-table">
        <thead>
        <tr>
          <th v-for="(item, index) in tableTitles" :key="index" @click="sortData(index)" :class="{active: sortTh === index}">
        ...
        <template v-for="(item, index) in infoList">
          ...
          <tr v-if="detailId === item.id">
            <td :colspan="tableTitles.length">
              <employee-detail :form-value="detail" :is-loading="detailLoading" :curr-id="detailId" @on-save="getDetail"></employee-detail>
            </td>
          </tr>
        </template>
        </tbody>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitPwd () {
        this.$http.post(Hr.basePwd, this.qs.stringify({password: this.pwd}), {headers: {'Authorization': 'Bearer ' + this.$cookie.get('token')}}).then((res) => {
          let _data = res.data
          if (_data.retCode === 0) {
            this.needPwd = false
            this.getList()
      ...
      getDetail () {
        ...
        this.$http.post(Hr.detailBasic, this.qs.stringify({id: this.detailId}), {headers: {'Authorization': 'Bearer ' + this.$cookie.get('token')}}).then(res => {
          let _data = res.data
      ...
      exportBasic () {
        window.open('/' + Hr.basicExport + '/?Authorization=Bearer ' + this.$cookie.get('token'))
      },
      uploadSuccess (res) {
        let _data = res
        if (_data.retCode === 0) {
      ...
      getList (obj) {
        ...
        this.$http.post(Hr.basicList, this.qs.stringify(_data), {headers: {'Authorization': 'Bearer ' + this.$cookie.get('token')}}).then(res => {
          let _data = res.data
    ...
    created () {
      ...
      sessionStorage.setItem('subMenu', 'employee/basic')
      this.$store.dispatch('setCurrSubMenu', 'employee/basic')
    },
    beforeDestroy () {
