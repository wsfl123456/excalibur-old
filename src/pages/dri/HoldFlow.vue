  ...
  <div class="dri-detail-tabs">
    ...
    <template v-for="(i, k) in holdCheckList">
      <div :key="k" v-if="k + 1 === activeQName">
        <div v-if="i && i.holdFlowDetail && i.holdFlowDetail.length">
          <div class="calculate">
            Q{{ k + 1 }}{{ checkName
            }}{{ removeZero(i.holdFlowScore) }}分=（<span
              v-for="(project, index) in i.holdFlowDetail"
              :key="index"
              >{{ project.projectName }} {{ removeZero(project.score) }}分<span
                v-if="index < i.holdFlowDetail.length - 1"
              >
                +
              </span> </span
            >）/{{ i.holdFlowDetail.length }}
          </div>

          <div class="list">
            <div v-for="(item, index) in i.holdFlowDetail" :key="index">
              <div class="pro-name">
