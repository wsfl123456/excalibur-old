<template>
  <div class="report-page">
      ...
      <div class="form-box">
        <div class="input-item">
          ...
          <div class="input-inner">
            <el-select v-model="searchParams.brand" @change="getProjectList" filterable placeholder="请选择品牌">
              <el-option v-for="item in brands" :key="item.id" :value="item.id" :label="item.customerName"></el-option>
            </el-select>
          </div>
        </div>
        <div class="input-item">
        ...
        <div class="input-item">
          ...
          <div class="input-inner status">
            ...
            <span class="total-momey" >({{formatMoney(1, 1, Number(fuState_1_total) + Number(fuState_2_total))}})</span>
    ...
    <div class="pagination-wrapper">
      <div class="record">
        <span class="font">显示第 {{ pagination.size*(pagination.page - 1) + 1 }} 到 {{ pagination.size*pagination.page }} 条记录 , 总共{{ pagination.total }}条记录。每页显示</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getBranList() {
        const res = await this.$http.post(Brand.listByProject, this.qs.stringify({page: 1, pageSize: 1000, powerType: 1}), {headers: {Authorization: "Bearer " + this.$cookie.get("token")}});
      ...
      async queryTableData(isExport) {
        ...
        const queryParams = {
          ...
          : "",
        totime: totime
          ? `${totime.getFullYear()}-${totime.getMonth() +
              1}-${totime.getDate()}`
          : ""
        };

        
        const res = await this.$http.post(Task.cwarlist, this.qs.stringify(queryParams), {headers: {Authorization: "Bearer " + this.$cookie.get("token")}});
      ...
      async getProjectList() {
        ...
        const res = await this.$http.post(Project.list, this.qs.stringify({page: 1, pageSize: 1000, customerId: this.searchParams.brand, powerType: 1}), {headers: {Authorization: "Bearer " + this.$cookie.get("token")}});
        const projects = res.data.retData.data;

        if (Array.isArray(projects)) this.projects = [{name: '全部'}, ...projects];
      },
      async getDepartments() {
        const res = await this.$http.post(Task.userteamlist, this.qs.stringify({page: 1, pageSize: 1000, powerType: 1}), {headers: {Authorization: "Bearer " + this.$cookie.get("token")}});
