<template>
  <div class="fc-overhead">
    <div class="form">
      ...
      <el-select
        v-model="searchParams.fapiaoType"
        filterable
        placeholder="请选择类别"
        class="item"
      >
    ...
    <div v-loading="isLoading">
      <table class="table" cellspacing="0">
        ...
        <tbody class="body" id="overheadTbody">
          <div v-show="list.length" id="overheadTbodyContent">
            <tr v-for="item in list" :key="item.id">
              <td v-for="i in tableTitle" :key="i.key" :width="i.width">
                <template v-if="i.key === 'creatorEnName'">
                  <member-header
                    :user="{
                      name: item.creatorEnName,
                      headPic: item.creatorHeadPic,
                      id: item.creatorId,
                    }"
                  ></member-header>
                </template>
                <template v-else-if="i.key === 'AmountE6'">{{
                  formatMoney(item[i.key])
                }}</template>
...
export default {
  ...
  methods: {
    ...
    async getOverheadList(isExport) {
      this.isLoading = true;
      // this.list = [];
